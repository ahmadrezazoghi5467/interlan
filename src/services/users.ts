    ...
    public getProfileTab(user_id:any,key:string,force:boolean=false){
        ...
        if(loadedtabs && loadedtabs.indexOf(key) == -1 || force){
            ...
            if(this.tabobservable[key]) {
                ...
                let opt = this.auth.getUserAuthorizationHeaders();
                let url = `${this.config.baseUrl}user/profile/?tab=`+key+`&per_page=`+this.config.settings.per_view+`&paged=`+this.profileTabPaged[key];
                        ...
                        if(response.status == 400) {
                          return "FAILURE";
                            ...
                            if(body){ 
                                ...
                                if(key == 'attendance'){
                                    let paged = this.profileTabPaged[key]?this.profileTabPaged[key]:1;
                                    this.attendance.get_my_course(body,paged);
                                }
